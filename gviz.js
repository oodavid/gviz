...
var gviz = {};
gviz.init = function(e){
	// Lookup this script
	var s = $('script[src$="gviz.js"]');
	if(s.length){
		// Using the src, figure out the path to the server...
		var a		= $('<a>').prop('href', s.attr('src'));
...
gviz.error = function(e){
	// Context buster
	var that = this;
	// ...there is no image with that hash - post off the source data and let the server do its thing...
	$.ajax({
		type:			'POST',
		url:			gviz.host,
		crossDomain:	true,
		...
		success: function(responseData, textStatus, jqXHR){
			// The hashed image should now exist, refresh the image
			$(that).attr('src', $(that).attr('src') + '?' + (new Date().getTime()));
		},
		error: function (responseData, textStatus, errorThrown){
			// Oh well...
			if(console){
				console.warn('GViz: Could not render image');
			}
		}
	});
};
